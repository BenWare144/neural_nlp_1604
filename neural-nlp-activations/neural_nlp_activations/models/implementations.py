        ...
        def align_tokens(self, tokenized_sentences, sentences, max_num_words, additional_tokens, use_special_tokens):
            ...
            for token_index in tqdm(range(len(tokenized_sentences)), desc='token features'):
                ...
                try:
                    ...
                    print(f"len(tokenized_sentences): {len(tokenized_sentences)}")
                    print(f"len(sentences_chain): {len(sentences_chain)}")
                    print(f"token_index: {token_index}")
                    print(f"token_word: {token_word}")
                    print(f"sentence_index: {sentence_index}")
                    print(f"tokenized_sentences[token_index]: {tokenized_sentences[token_index]}")
                    print(f"additional_tokens: {additional_tokens}")
                    print("self.tokenizer_special_tokens:",self.tokenizer_special_tokens)
                    print(f"previous_indices: {previous_indices}")
                    print(f"sentences_chain: {sentences_chain}")
                    print(f"tokenized_sentences: {tokenized_sentences}")
                    print(f"sentences_chain[sentence_index-1].lower(): {sentences_chain[sentence_index-1].lower()}")
                    print(f"tokenized_sentences[token_index-1]: {tokenized_sentences[token_index-1]}")
                    assert 2==1
                    continue
                previous_indices = []
                sentence_index += 1
...
from pathlib import Path
def list_custom_identifiers(weight_dir="/home/ben/data/transformer_weights/"): return [x.stem for x in Path(weight_dir).iterdir() if x.is_file()]
def get_base_model_from_identifier(x): return x.split("_")[0]
def list_presaved_models(saved_model_dir="/home/ben/data/transformer_saved_models"):
    return [x.stem for x in Path(saved_model_dir).iterdir() if not x.stem=="old"]
def get_identifier_from_configurations(base_model, transformer_configurations):
    for configuration in transformer_configurations:
        configuration = copy.deepcopy(configuration)
        identifier = configuration.get('identifier', configuration['weight_identifier']) + '-untrained'
        if identifier == base_model:
...
#             # save_loc=f"/home/ben/data/transformer_saved_models/E1_Schrimpfs_{model}"
#             # print(f"save loc={save_loc}")
#             # model_impl.save_pretrained(save_loc)
#             # print("="*25)
...
#         # if identifier in ["gpt2","gpt2-xl"]:
#             # print(f"identifier is {identifier} | untrained is {untrained} | not configuration.get('trained', True) is {not configuration.get('trained', True)}")

#         # if identifier in ["gpt2-untrained","gpt2-xl-untrained"] and untrained: # if untrained
#         if untrained and "gpt2" in identifier: # if untrained
#         # if untrained and any(x in identifier for x in ["gpt2_","gpt2-xl"]): # if untrained
#             # print(f"identifier is {identifier} | untrained is {untrained} | not configuration.get('trained', True) is {not configuration.get('trained', True)}")
#             identifier=identifier.replace("-untrained","")
#             # print(f"mycode active: true")
#             variants=["default","control","1aa","1ab","1ac","1ba","1bb","1bc","1ca","1cb","1cc","2aa","2ab","2ac","2ba","2bb","2bc","2ca","2cb","2cc"]
#             # variants+=["random"]
#             custom_identifiers=[f"{identifier}_TH_weight_config_{v}" for v in variants]
#             for custom_identifier in custom_identifiers:
#                 # print(f"custom_identifier: {custom_identifier}")
#                 def model_instantiation(identifier=custom_identifier, configuration=frozenset(configuration.items())):
#                     configuration = dict(configuration)  # restore from frozen
#                     module = import_module('transformers')
#                     config_ctr = getattr(module, configuration['config_ctr'])
#                     model_ctr = getattr(module, configuration['model_ctr'])
...
#                     model = model_ctr.from_pretrained(configuration['weight_identifier'],
#                                                     output_hidden_states=True, state_dict=state_dict)
#                     load_weight_config_from_local_file(model, file_name=identifier+".pt")
#                     model_wrapper = configuration.get('model_wrapper', None)
#                     if model_wrapper:
#                         model = model_wrapper(model)
#                     transformer = _PytorchTransformerWrapper(
...
#                         sentence_average=word_last)
#                     return transformer
#                 model_pool[custom_identifier] = LazyLoad(model_instantiation)
#                 model_layers[custom_identifier] = list(configuration['layers'])


custom_identifiers = list_custom_identifiers(weight_dir="/home/ben/data/transformer_weights/")
presaved_models = list_presaved_models(saved_model_dir="/home/ben/data/transformer_saved_models")
configs=[]
for untrained in False, True:
    for configuration in transformer_configurations:
        configuration = copy.deepcopy(configuration)
        ...
        configs.append((identifier, custom_identifier, configuration))
for custom_identifier in custom_identifiers: 
    base_model = get_base_model_from_identifier(custom_identifier)
    configuration, identifier = get_identifier_from_configurations(base_model, transformer_configurations)
...
for identifier, custom_identifier, configuration in configs:
    def model_instantiation(identifier=identifier, custom_identifier=custom_identifier, configuration=frozenset(configuration.items())):
        configuration = dict(configuration)  # restore from frozen
        module = import_module('transformers')
        config_ctr = getattr(module, configuration['config_ctr'])
        model_ctr = getattr(module, configuration['model_ctr'])
        ...
        config = config_ctr.from_pretrained(configuration['weight_identifier'])
        tokenizer = tokenizer_ctr.from_pretrained(configuration['weight_identifier'])
        # print(f"presaved_models: {presaved_models}")
        if f"E1_Schrimpfs_{identifier}" in presaved_models:
            saved_model_loc=f"/home/ben/data/transformer_saved_models/E1_Schrimpfs_{identifier}"
            print(f"loading presaved model {identifier} from {saved_model_loc}")
            model = model_ctr.from_pretrained(saved_model_loc, output_hidden_states=True)
        else:
            ...
            model = model_ctr.from_pretrained(configuration['weight_identifier'],
                                                output_hidden_states=True, state_dict=state_dict)
        if custom_identifier != identifier:
            print(f"running load_weight_config_from_local_file")
            print(f"identifier: {identifier}")
            print(f"custom_identifier: {custom_identifier}")
            print(f"file_name: {custom_identifier}.pt")
            load_weight_config_from_local_file(model, file_name=f"{custom_identifier}.pt")
